...
除了墙面，我们还需要处理怪物的贴图，然而这就需要正确获知渲染墙体/怪物之间的遮挡关系，才能最终实现3D渲染。为此，我们在 MASM 32 中手动实现了一套快速排序算法，将每一帧的所有待渲染物体按照与玩家的距离从远到近渲染。
...
支持多张地图的加载，当一张地图的所有怪物死亡后进入下一张地图，进入下一张地图后有1秒时间显示"NEXT STAGE"提示，没有下一张地图则提示胜利"VICTORY"。
