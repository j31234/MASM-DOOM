...
MASM32 库中提供了`gdi.inc`与`gdi.lib`，包含了[Windows graphics device interface](https://learn.microsoft.com/en-us/windows/win32/gdi/windows-gdi)（GDI）。GDI 是 Window 提供的低级绘图接口，提供了若干接口用于绘制画面：
...
为了正确渲染墙体/怪物之间的遮挡关系，我们利用在 MASM32 中手动实现的快速排序，将每一帧的所有待渲染物体按照与玩家的距离从远到近渲染。对于不同的墙体、怪物动画、武器动画与 UI 界面，我们均用贴图技术实现。
...
由于 PlaySound 函数不支持在一个进程中同时播放多个音乐，我们使用多进程播放音乐，保证多个音效可以和背景音乐同时播放。

我们为玩家受攻击，玩家死亡，武器开火，怪物受攻击，怪物死亡，游戏结束等多个事件添加了复古风格的音效。
...
- 怪物血量：当武器开火时，用 Ray Casting 算法发射射线，检测武器能否击中怪物，并扣除血量
  - 怪物死亡后不再移动
...
一种解决方案是在用`/SUBSYSTEM:CONSOLE`链接子系统的 MASM 程序中调用`GetConsoleWindow`获取控制台 handler，然后调用`ShowWindow`隐藏窗口，但是这种方法不能隐藏 Windows 11 的默认终端。

另一个解决方案是用`/SUBSYSTEM:WINDOWS`链接子系统，且不使用`CreateWindow`函数创建窗口。这样创建的新进程可以完美地在后台运行。
...
| 董文冲 | 背景音乐，武器/怪物/玩家音效，怪物血量/受击，自定义地图     |
